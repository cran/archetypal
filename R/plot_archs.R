plot_archs=function(archs, data = NULL, show_data = FALSE, ...){
  # Check data existence
  if(show_data & is.null(data)){
    stop("You must provide the data set if you have chosen  to show it ...")
  }
  # Plot given set of archetypes with or without showing the data set
  # Data frame
  ...
  if(nd==1){
    ...
    if(show_data){
      ...
      parmarold=par("mar")
      par(mar=c(0,0,0,0))
      plot(dp12, pch = 19, col="gray40", cex=1.5, axes=F, xlab="",ylab="",yaxs="i", main = "", bty="n")
      xtiks=as.integer(dp12[,1])
      xlabs=round(xtiks,digits = 1)
      abline(h=0)
    }
    # Multiple labels
  ...
  if(nd==2){
    ...
    if(show_data){
      ...
      plot(da,pch=19,cex=01.25,xlab="",ylab="")
    }
    colarchs = "gray40"
  ...
  if(nd>=4){
    ...
    segs=5
    # Create unique points to show, if requested so
    if(show_data){
      # Keep only unique points for plot
      dtlist=lapply(1:dim(df)[2], function(i,df){
        x=as.numeric(df[,i])
        dt=data.frame(table(x))
        dt$x=as.numeric(as.character(dt$x))
        dt$rf=dt$Freq/sum(dt$Freq)
